...
typedef union {
  void  (*w32) (GFP gf, void *src, void *dest, gf_val_32_t val,
                int bytes, int add);
  void  (*w64) (GFP gf, void *src, void *dest, gf_val_64_t val,
                int bytes, int add);
  void  (*w128)(GFP gf, void *src, void *dest, gf_val_128_t val,
                int bytes, int add);
} gf_region;
...
typedef union {
  ...
  void         (*w128)(GFP gf, void *start, int bytes, int index,
                           gf_val_128_t rv);
} gf_extract;
...
#define GF_W16_INLINE_MULT(log, alog, a, b) \\
  ((a) == 0 || (b) == 0) ? 0 : (alog[(uint32_t)log[a]+(uint32_t)log[b]])
#define GF_W16_INLINE_DIV(log, alog, a, b) \\
  ((a) == 0 || (b) == 0) ? 0 : (alog[(int)log[a]-(int)log[b]])
#endif
